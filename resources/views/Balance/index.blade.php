                ...
                <div class="card">
                    <div class="card-header"
                        ...
                        <a href="{{route('balance-create')}}">
                            <button type="button" class="btn btn-info btn-lg" style="display: flex ; align-items: center;">
                        ...
                        <div class="table-wrapper">
                            <div class="table-responsive">
                                <table class="table table-striped  table-bordered table-hover" id="table">
                                    ...
                                    <tbody>
                                    @foreach ( $weightStatments as $balance )
                                        <tr>
                                            <td class="text-center"> {{ $loop -> index + 1 }} </td>
                                            <td class="text-center"> {{ $balance -> meal }} </td>
                                            <td class="text-center"> {{ $balance -> client }} </td>
                                            <td class="text-center"> {{ \Carbon\Carbon::parse($doc -> date) ->format('Y-m-d') }} </td>
                                            <td class="text-center"> {{ $balance -> quantity }} </td>
                                            <td class="text-center"> {{ $balance -> quantity / 4 }} </td>
                                            <td class="text-center"> {{ $balance -> net_weight }} </td>
                                            <td class="text-center"> {{ $balance -> total }} </td>
...
<script >
    ...
    function confirmDelete(){
        let url = "{{ route('balance-destroy', ':id') }}";

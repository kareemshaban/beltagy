        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title fw-semibold " style="margin-bottom: 0 !important; color: #3cacc8 ">تقرير كشف التمليح</h5>
                            ...
                            @foreach ( $data as $meal )
                                ...
                                @if(count($meal -> exits) > 0)
                                    ...
                                    <td colspan="9">
                                        <table class="table table-danger table-striped  table-bordered " >
                                            <thead>
                                            <tr>
                                                <td colspan="10" class="text-center text-primary">  رسائل الخروج من {{ $meal -> item_name }} -- {{$meal -> item_code}} </td>
                                            </tr>
                                            <tr>
                                                <th class="text-center"> # </th>
                                                <th class="text-center"> {{ __('main.client') }} </th>
                                            ...
                                            @foreach ( $meal -> exits as $exit )
                                                <tr>
                                                    ...
                                                    <td class="text-center"> {{ $exit -> weight}} </td>
                                                    <td class="text-center"> {{ $exit -> duration}} </td>
                                                    <td class="text-center"> {{ $exit -> serviceTotal}} </td>
                                                </tr>
                                            @endforeach
                                ...
                                @else
                                    <tr style="background: #0a53be">
                                        ...
                                        <td colspan="9" class="text-danger text-center"> لا يوجد رسائل خروج </td>
                                    </tr>
                                @endif
                                <tr>
                                    ...
                                    <td colspan="3"  class="text-center text-success">  {{($meal -> weight - $meal -> outingWeight)}}  KG </td>
